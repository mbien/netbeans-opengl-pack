 ...
 */
public class ShaderVariableEditor extends JPanel implements TableCellEditor {
    ...
    protected ShaderVariable value;
    
    
    /**
     * Creates a new instance of ShaderVariableEditor
     */
    public ShaderVariableEditor() {
    ...
    public void setDataType(int dataType) {
        ...
        // If the dimension is not set at all it's set by default to DATA_TYPE_DIMENSION1
        if (DataType.getDimensionOnly(dataType) == 0)
    ...
    protected void createInputFields(int dimension) {
        ...
        VariableValue value = this.value.getVariableValue();
        ...
        if ((chosenDataTypeExplicit &
            ...
            dynamicContent.setLayout(new GridLayout(0, dimension, 2, 2));
            for (int i = 0; i < dimension; i++) {
                switch (chosenDataTypeExplicit) {
                    case DataType.DATA_TYPE_BOOL:
                        JCheckBox box = new JCheckBox(getFieldDescription(i));
                        box.setSelected(value.getBooleanValue(i));
                        dynamicContent.add(box);
                        break;
                    case DataType.DATA_TYPE_INT:
                        ...
                        textFieldInt.setValue(value.getIntegerValue(i));
...
//                        Float[] matValue = value.getMatrixValue(i);
    ...
    private void refreshCellSize() {
        ...
        if (this.getPreferredSize().getHeight() > 0)
            this.table.setRowHeight(row, (int) this.getPreferredSize().getHeight());
        if (this.getPreferredSize().getWidth() > 0)
    ...
    public Object getCellEditorValue() {
        ...
        newValue.setDataType(chosenDataTypeExplicit | chosenDataTypeDimension);
        ...
        if ((chosenDataTypeExplicit &
                ...
                > 0) {
            // Sampler
            //TODO: set Sampler value
        } else {
            Object v;
            for (int i = 0; i < dimension; i++) {
                switch (chosenDataTypeExplicit) {
                    case DataType.DATA_TYPE_BOOL:
                        newValue.setValue(i, ((JCheckBox) dynamicContent.getComponent(i)).isSelected());
                        break;
                    case DataType.DATA_TYPE_INT:
                        v = ((JFormattedTextField) dynamicContent.getComponent(i)).getValue();
                        newValue.setValue(i, v == null ? 0 : ((Number) v).intValue());
                        break;
                    case DataType.DATA_TYPE_FLOAT:
                        v = ((JFormattedTextField) dynamicContent.getComponent(i)).getValue();
                        newValue.setValue(i, v == null ? 0f : ((Number) v).floatValue());
                        break;
                    case DataType.DATA_TYPE_MAT:
                        Float[] matValues = new Float[dimension];
                        for (int j = 0; j < dimension; j++) {
                            v = ((JFormattedTextField) dynamicContent.getComponent(i * dimension + j)).getValue();
                            matValues[j] = v == null ? 0f : ((Number) v).floatValue();
                        }
                        newValue.setValue(i, matValues);
                        break;
                }
            }
        }
    ...
    protected VariableValue getVariableValue() {
        return (this.value.getVariableValue() == null) ? 
                    new VariableValue() :
                    this.value.getVariableValue();
    ...
    protected void createControls() {
        setDataType(this.value.getDataType());
        presetDataTypeChoice(this.value.getChosenDataType());
    }
    
    private void presetDataTypeChoice(int dataType) {
        if (boxDataType != null)
            boxDataType.setSelectedIndex(DataType.getDataTypeIndex(dataType));
        if (boxDataTypeDimension != null)
            boxDataTypeDimension.setSelectedIndex(DataType.getDimensions(dataType) - 1);
    }
